    ...
    <form action="" id="manage_playlist_items">
        ...
        <div class="row">
            <div class="form-group col-md-12">
                <label for="" class="control-label">尋找音樂</label>
...
?>
                    <tr data-id='<?php echo $row['id'] ?>'>
                        ...
                        <td><input name='music_id[]' value='<?php echo $row['id'] ?>' type='hidden'><i
                                class='fa fa-music text-gradient-primary mr-2'></i><?php echo $row['title'] ?></td>
    ...
    <li class="list-group-item suggest-item">
        <div id="pdet" class="d-flex justify-content-between align-items-center">
            <div class="d-flex">
                ...
                <div class="ml-2 mr-4">
                    <div><b>
                            <large class="mtitle">標題</large>
                        </b>
                    </div>
                    <div>
                        <b><small class="martist">作曲者</small></b>
                    </div>
                </div>
            </div>
            </span>
                ...
                if (resp) {
                    ...
                    if (resp.lenght <= 0) {
                        ...
                        Object.keys(resp).map(k => {
                            ...
                            li.find('img').attr('src', 'assets/uploads/' + resp[k]
                                .cover_image)
                            li.find('.mtitle').text(resp[k].title)
                            li.find('.martist').text(resp[k].artist)
                            li.find('.bp').attr('onclick', "play_music({0:{id:" + resp[k]
                                .id + ",upath:'assets/uploads/" + resp[k].upath + "'}})"
                                )
...
function li_func() {
    ...
    $('.suggest-item').click(function() {
        ...
        tr.append(
            '<td><span class="btn bg-gradient-success rounded-circle d-flex justify-content-center align-items-center" style="width:30px;height:30px;z-index:2" onclick="play_music({0:{id:' +
            data.id + ',upath:\'assets/uploads/' + data.upath +
            '\'}})"><div class="fa fa-play text-white"></div></span></td>')
        tr.append("<td><input name='music_id[]' value='" + data.id +
            "' type='hidden'><i class='fa fa-music text-gradient-primary'></i>" + data.title + "</td>")
        tr.append(
